...
namespace kxf
{
	...
	{
		return m_Renderer->Get().CreateFont(m_CurrentFont.AsWXFont(), m_CurrentFontColor);
	}
	wxGraphicsFont WxGraphicsContext::MakeGCFont(const IGraphicsFont& font, const Color& color) const
	{
		...
		return m_Renderer->Get().CreateFont(gdiFont.AsWXFont(), color);
	}
		...
		if (!rect.IsEmpty() && texture)
		{
			if (auto textureWx = texture.QueryInterface<WxGraphicsTexture>())
			{
				...
				else
				{
					...
					m_Context->DrawBitmap(m_Renderer->Get().CreateBitmapFromImage(image.AsWXImage()), rect.GetX(), rect.GetY(), rect.GetWidth(), rect.GetHeight());
				}
	...
	void WxGraphicsContext::DrawTexture(const BitmapImage& image, const RectF& rect)
	{
		...
		else
		{
			...
			m_Context->DrawBitmap(m_Renderer->Get().CreateBitmapFromImage(temp.AsWXImage()), rect.GetX(), rect.GetY(), rect.GetWidth(), rect.GetHeight());
		}
	}
	...
	RectF WxGraphicsContext::DrawLabel(const String& text, const RectF& rect, const IGraphicsTexture& icon, const IGraphicsFont& font, const IGraphicsBrush& brush, FlagSet<Alignment> alignment, size_t acceleratorIndex)
	{
		if (!rect.IsEmpty() && (!text.IsEmpty() || icon))
		{
			...
			if (icon)
			{
				...
				{
					boundingBox = m_GCDC.DrawLabel(text, rect, iconWx->GetImage().ToWXBitmap(), alignment, acceleratorIndex);
				}
				else
				{
					boundingBox = m_GCDC.DrawLabel(text, rect, icon.ToBitmapImage().ToWXBitmap(), alignment, acceleratorIndex);
				}
			}
			else
			{
	...
	void WxGraphicsContext::DrawGDI(const RectF& rect, std::function<void(GDIContext& dc)> func)
	{
		...
		{
			m_Context->DrawBitmap(bitmap.AsWXBitmap(), rect.GetX(), rect.GetY(), rect.GetWidth(), rect.GetHeight());
			CalcBoundingBox(rect);
		}
	}
...
namespace kxf
{
	void WxGraphicsGDIContext::Initialize(WxGraphicsRenderer& rendrer, wxDC& dc, const Size& size)
	{
		...
		WxGraphicsContext::Initialize(rendrer, std::unique_ptr<wxGraphicsContext>(rendrer.Get().CreateContextFromImage(m_Image->AsWXImage())));
	}
	...
	bool WxGraphicsGDIContext::FlushContent()
	{
		...
		{
			m_Context->Flush();
			m_DC.DrawBitmap(m_Image->ToWXBitmap(), {0, 0});
