...
namespace kxf
{
	class KXF_API GDIContext: public IGDIObject
	{
		...
		public:
			...
			// GDIContext
			const wxDC& AsWXDC() const noexcept
			{
				return *m_DC;
			}
			wxDC& AsWXDC() noexcept
			{
				return *m_DC;
			}
			...
			{
				m_DC->DrawIcon(icon.AsWXIcon(), pos);
			}
			void DrawBitmap(const GDIBitmap& bitmap, const Point& pos)
			{
				m_DC->DrawBitmap(bitmap.AsWXBitmap(), pos, false);
			}
			...
			{
				wxSize size;
				m_DC->GetTextExtent(text, &size.x, &size.y, nullptr, nullptr, &font.AsWXFont());
			...
			{
				wxSize size;
				m_DC->GetMultiLineTextExtent(text, &size.x, &size.y, nullptr, &font.AsWXFont());
