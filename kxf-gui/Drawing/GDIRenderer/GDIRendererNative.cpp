...
namespace kxf
{
	...
	void GDIRendererNative::DrawCheckBox(wxWindow* window, GDIContext& dc, const Rect& rect, FlagSet<NativeWidgetFlag> widgetFlags)
	{
		...
		{
			return GetRenderer().DrawRadioBitmap(window, dc.AsWXDC(), rect, *MapWidgetFlags(widgetFlags));
		}
		else
		{
			return GetRenderer().DrawCheckBox(window, dc.AsWXDC(), rect, *MapWidgetFlags(widgetFlags));
		}
	}
	...
	{
		CalcBoundingBox calcBoudingBox(dc, rect);
		return GetRenderer().DrawComboBoxDropButton(window, dc.AsWXDC(), rect, *MapWidgetFlags(widgetFlags));
	}
	...
	{
		CalcBoundingBox calcBoudingBox(dc, rect);
		return GetRenderer().DrawCollapseButton(window, dc.AsWXDC(), rect, *MapWidgetFlags(widgetFlags));
	}
	...
	{
		CalcBoundingBox calcBoudingBox(dc, rect);
		return GetRenderer().DrawPushButton(window, dc.AsWXDC(), rect, *MapWidgetFlags(widgetFlags));
	}
	...
	void GDIRendererNative::DrawExpanderButton(wxWindow* window, GDIContext& dc, const Rect& rect, FlagSet<NativeWidgetFlag> widgetFlags)
	{
		if (window)
		{
			...
			{
				GetRenderer().DrawTreeItemButton(window, dc.AsWXDC(), rect, *MapWidgetFlags(widgetFlags));
			}
		}
	}
	...
	void GDIRendererNative::DrawProgressMeter(wxWindow* window, GDIContext& dc, const Rect& rect, int value, int range, FlagSet<NativeWidgetFlag> widgetFlags)
	{
		if (window)
		{
			...
			{
				GetRenderer().DrawGauge(window, dc.AsWXDC(), rect, value, range, *MapWidgetFlags(widgetFlags));
			}
		}
	}
	...
	void GDIRendererNative::DrawItemSelectionRect(wxWindow* window, GDIContext& dc, const Rect& rect, FlagSet<NativeWidgetFlag> widgetFlags)
	{
		if (window)
		{
			...
			{
				GetRenderer().DrawItemSelectionRect(window, dc.AsWXDC(), rect, *MapWidgetFlags(widgetFlags));
			}
		}
	}
	void GDIRendererNative::DrawItemFocusRect(wxWindow* window, GDIContext& dc, const Rect& rect, FlagSet<NativeWidgetFlag> widgetFlags)
	{
		if (window)
		{
			...
			{
				GetRenderer().DrawFocusRect(window, dc.AsWXDC(), rect, *MapWidgetFlags(widgetFlags));
			}
		}
	}
	...
	{
		CalcBoundingBox calcBoudingBox(dc, rect);
		GetRenderer().DrawItemText(window, dc.AsWXDC(), text, rect, *wxWidgets::MapAlignment(alignment), *MapWidgetFlags(widgetFlags), static_cast<wxEllipsizeMode>(ellipsizeMode));
	}
	...
	void GDIRendererNative::DrawTitleBarButton(wxWindow* window, GDIContext& dc, const Rect& rect, NativeTitleBarButton button, FlagSet<NativeWidgetFlag> widgetFlags)
	{
		...
		{
			CalcBoundingBox calcBoudingBox(dc, rect);
			GetRenderer().DrawTitleBarBitmap(window, dc.AsWXDC(), rect, button, *MapWidgetFlags(widgetFlags));
		};
	...
	void GDIRendererNative::DrawComboBox(wxWindow* window, GDIContext& dc, const Rect& rect, FlagSet<NativeWidgetFlag> widgetFlags)
	{
		...
		{
			GetRenderer().DrawComboBox(window, dc.AsWXDC(), rect, *MapWidgetFlags(widgetFlags));
		}
		else
		{
			GetRenderer().DrawChoice(window, dc.AsWXDC(), rect, *MapWidgetFlags(widgetFlags));
		}
	}
	...
	{
		CalcBoundingBox calcBoudingBox(dc, rect);
		GetRenderer().DrawTextCtrl(window, dc.AsWXDC(), rect, *MapWidgetFlags(widgetFlags));
	}
	...
	{
		CalcBoundingBox calcBoudingBox(dc, rect);
		GetRenderer().DrawSplitterBorder(window, dc.AsWXDC(), rect, *MapWidgetFlags(widgetFlags));
	}
	void GDIRendererNative::DrawSplitterSash(wxWindow* window, GDIContext& dc, const Rect& rect, int position, FlagSet<NativeWidgetFlag> widgetFlags)
	{
		...
		CalcBoundingBox calcBoudingBox(dc, rect);
		GetRenderer().DrawSplitterSash(window, dc.AsWXDC(), rect.GetSize(), position, widgetFlags.Contains(NativeWidgetFlag::Vertical) ? wxVERTICAL : wxHORIZONTAL);
	}
	...
	)
	{
		...
		if (buttonInfo)
		{
			...
			return GetRenderer().DrawHeaderButton(window, dc.AsWXDC(), rect, *MapWidgetFlags(widgetFlags), MapSortArrow(sortArrow), &parameters);
		}
		else
		{
			return GetRenderer().DrawHeaderButton(window, dc.AsWXDC(), rect, *MapWidgetFlags(widgetFlags), MapSortArrow(sortArrow));
		}
	}
	...
	)
	{
		...
		{
			wxHeaderButtonParams parameters = ConvertHeaderButtonInfo(*buttonInfo);
			return GetRenderer().DrawHeaderButtonContents(window, dc.AsWXDC(), rect, *MapWidgetFlags(widgetFlags), MapSortArrow(sortArrow), &parameters);
		}
		else
		{
			return GetRenderer().DrawHeaderButtonContents(window, dc.AsWXDC(), rect, *MapWidgetFlags(widgetFlags), MapSortArrow(sortArrow));
		}
	}
}
