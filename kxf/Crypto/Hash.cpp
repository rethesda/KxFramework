...
namespace
{
	...
	template<size_t hashLength, class TInitFunc, class TFreeFunc, class TResetFunc, class TUpdateFunc, class TFinalFunc, class TSeed = uint32_t, class TValue = uint8_t>
	...
	) noexcept
	{
		if (auto* state = std::invoke(initFunc))
		{
			...
			if (std::invoke(resetFunc, state, seed) != XXH_ERROR)
			{
...
namespace kxf::Crypto
{
	HashValue<32> xxHash_32(const void* data, size_t size, uint32_t seed) noexcept
	{
		return XXH32(data, size, seed);
	}
	HashValue<64> xxHash_64(const void* data, size_t size, uint64_t seed) noexcept
	{
		return seed != 0 ? XXH3_64bits_withSeed(data, size, seed) : XXH3_64bits(data, size);
	}
	HashValue<128> xxHash_128(const void* data, size_t size, uint64_t seed) noexcept
	{
		XXH128_hash_t result = seed != 0 ? XXH3_128bits_withSeed(data, size, seed) : XXH3_128bits(data, size);
		return HashValue<128>(&result, sizeof(result));
	}

	HashValue<32> xxHash_32(IInputStream& stream, uint32_t seed) noexcept
	{
		return DoCalcXXHash<32>(stream, XXH32_createState, XXH32_freeState, XXH32_reset, XXH32_update, XXH32_digest, seed);
	}
	HashValue<64> xxHash_64(IInputStream& stream, uint64_t seed) noexcept
	{
		return DoCalcXXHash<64>(stream, XXH3_createState, XXH3_freeState, XXH3_64bits_reset_withSeed, XXH3_64bits_update, XXH3_64bits_digest, seed);
	}
	HashValue<128> xxHash_128(IInputStream& stream, uint64_t seed) noexcept
	{
		return DoCalcXXHash<128>(stream, XXH3_createState, XXH3_freeState, XXH3_128bits_reset_withSeed, XXH3_128bits_update, XXH3_128bits_digest, seed);
	}
}
